...
//        NSLog(@"%f,%lu",compression,(unsigned long)newimageData.length);
        compression -= scale;
        NSLog(@"压缩图片-->%f-->%@-->%ld",compression,newimageData,(long)FileSize);
    }
...
+ (UIImage *)OnlyCompressToImageWithImageCopy:(UIImage *)OldImage FileSize:(NSInteger)FileSize {
    NSData * imagedata = [self OnlyCompressToDataWithImage:OldImage FileSize:FileSize];
    UIImage *compressedImage = [UIImage imageWithData:imagedata];
    NSLog(@"结束-->只压不缩-返回Data大小：%lu kb",imagedata.length/1024);
    return compressedImage;
}
                               ...
                               FileSize:(NSInteger)FileSize
{
    ...
    NSData *imageData = UIImageJPEGRepresentation(OldImage,
                                                  compression);
    NSLog(@"开始-->只压不缩-返回Data大小：%lu kb",imageData.length/1024);
    //每次减少的比例
    float scale = 0.1;
