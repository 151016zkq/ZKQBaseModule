...
+ (UILabel *)creatWithFont:(CGFloat)font TextLightColor:(NSString *)color darkColor:(NSString *)darkColor {
    UILabel *label = [[UILabel alloc] init];
    label.font = [UIFont systemFontOfSize:font];
    label.textColor = DarkColorHex(color, darkColor);
    label.numberOfLines = 0;
    return label;
}
//带有暗黑模式字体颜色
+ (UILabel *)creatWithFont:(CGFloat)font TextColor:(NSString *)color darkColor:(NSString *)darkColor Text:(NSString *)text {
    UILabel *label = [self creatWithFont:font TextLightColor:color darkColor:darkColor];
    label.numberOfLines = 0;
    label.text = text;
    return label;
}
...
+ (UILabel *)speratorLineOffSet:(CGFloat)offSetY {
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, offSetY, [UIScreen mainScreen].bounds.size.width, 10)];
    label.backgroundColor = KBackColorSwift;
    return label;
}
